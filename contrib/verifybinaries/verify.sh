...
###   deftchaincore.org and deftchain.org and compares them.
...
WORKINGDIR="/tmp/deftchain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://deftchaincore.org"
HOST2="https://deftchain.org"
BASEDIR="/bin/"
VERSIONPREFIX="deftchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'deftchain-',
...
if [ $? -ne 0 ]; then
   echo "deftchain.org failed to provide signature file, but deftchaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "deftchain.org and deftchaincore.org signature files were not equal?"
