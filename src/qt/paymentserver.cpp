...
const int DEFTCHAIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString DEFTCHAIN_IPC_PREFIX("deftchain:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/deftchain-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/deftchain-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/deftchain-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the deftchain: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(DEFTCHAIN_IPC_PREFIX, Qt::CaseInsensitive)) // deftchain: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(DEFTCHAIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(DEFTCHAIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click deftchain: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start deftchain: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling deftchain: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in deftchain: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("deftchain://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'deftchain://' is not a valid URI. Use 'deftchain:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(DEFTCHAIN_IPC_PREFIX, Qt::CaseInsensitive)) // deftchain: URI
    {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom deftchain addresses are not supported
