...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Core](#general-deftchain-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play deftchains" on the test network, if you
...
([`valgrind.supp`](https://github.com/deftchain/deftchain/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_deftchain
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_deftchain --log_level=test_suite
$ valgrind -v --leak-check=full src/deftchaind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_deftchain.coverage/index.html`.
```
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/deftchain/deftchain/issues/12691)
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `DEFTCHAIN_FOO_BAR_H`, e.g.

```c++
#ifndef DEFTCHAIN_FOO_BAR_H
#define DEFTCHAIN_FOO_BAR_H
...
#endif // DEFTCHAIN_FOO_BAR_H
```
...
- src/libsecp256k1
  - Upstream at https://github.com/deftchain-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/deftchain-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:deftchain/deftchain.git
...
Commit [`bb81e173`](https://github.com/deftchain/deftchain/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `deftchain-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `deftchain-cli` and the GUI debug console use this table to determine how to
